...
public class UpdateManager {
    ...
    private boolean isNewVersion(String localVersionName, String remoteVersionName) {
        if (null == localVersionName || null == remoteVersionName) {
            return false;
        }

        if (localVersionName.equals("") || remoteVersionName.equals("")) {
            return false;
        }

        String[] local = localVersionName.split("\\.");
        String[] remote = remoteVersionName.split("\\.");

        int nLocalSize = local.length;

        Integer[] intLocal = new Integer[nLocalSize];
        Integer[] intRemote = new Integer[nLocalSize];

        for (int i = 0; i < nLocalSize; ++i) {
            intLocal[i] = Integer.parseInt(local[i]);
            intRemote[i] = Integer.parseInt(remote[i]);

            if (intRemote[i] > intLocal[i]) {
                return true;
            }
        }
        return false;
